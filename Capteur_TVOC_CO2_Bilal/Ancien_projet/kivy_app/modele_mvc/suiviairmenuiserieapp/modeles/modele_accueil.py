...
class ModeleAccueil:
    ...
    def _init_mqtt_seuils(self):
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                if maj:
                    self.seuils_mis_a_jour = True  # Signale la mise à jour
    ...
    def verifier_seuils(self):
        ...
        # Logique : vert si < limite_seuil, rouge si >= limite_seuil
        def get_alerte(valeur, seuil):
            if valeur is None:
                return "indisponible"
            if valeur < seuil["limite_seuil"]:
                return "normal"
            else:
                return "danger"

        alertes = {
            "co2": get_alerte(self.co2, seuil_co2),
            "tvoc": get_alerte(self.tvoc, seuil_tvoc),
    ...
    def reset_seuils_mis_a_jour(self):
        """Remet à False le flag après affichage."""
