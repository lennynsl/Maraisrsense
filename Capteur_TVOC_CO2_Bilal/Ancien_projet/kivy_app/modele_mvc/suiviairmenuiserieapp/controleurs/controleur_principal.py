...
class Controleur:
    ...
    def mettre_a_jour_affichage_seuils(self, *args):
        ...
        if self.ihm_accueil is not None:
            self.ihm_accueil.afficher_co2_tvoc(co2, tvoc, alertes, moyenne_jour, seuils_mis_a_jour=seuils_mis_a_jour)
            if seuils_mis_a_jour:
                # Réinitialise le flag après 4 secondes
                from kivy.clock import Clock
                Clock.schedule_once(lambda dt: self._reset_seuils_mis_a_jour(), 4)
        else:
            print("ihm_accueil n'est pas défini. Assurez-vous que set_ihm_accueil a été appelé.")

    def _reset_seuils_mis_a_jour(self):
        self.gestion_accueil.reset_seuils_mis_a_jour()
        # Rafraîchit l'affichage pour masquer le message
        self.mettre_a_jour_affichage_seuils()
