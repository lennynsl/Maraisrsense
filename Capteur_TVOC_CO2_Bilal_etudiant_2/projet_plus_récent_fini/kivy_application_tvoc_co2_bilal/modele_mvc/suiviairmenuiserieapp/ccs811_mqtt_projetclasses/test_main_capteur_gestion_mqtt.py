"""
test_main_capteur_gestion_mqtt.py
---------------------------------
Script de test pour la lecture du capteur CCS811 et l'envoi des mesures sur un broker MQTT.
Permet de vérifier la communication capteur <-> MQTT indépendamment de l'application principale.

Ce script est destiné à des tests unitaires ou de diagnostic matériel/logiciel.
Il n'est pas utilisé dans l'application principale.

La boucle principale lit les mesures du capteur CCS811 toutes les 5 secondes et les publie sur le broker MQTT.
Si une erreur survient, elle est affichée et une nouvelle tentative est effectuée après une courte pause.

Utilisation :
    Lancez ce script pour tester la lecture du capteur et la publication MQTT en boucle.
"""
...
def main():
    """
    Fonction principale de test : initialise le capteur CCS811 et publie les mesures sur MQTT.
    """
